		...
		for (float theta_deg = 0; theta_deg < 180; theta_deg += theta_deg_step) {
			...
			for (auto& vertex : vertices) {
				...
				if (vertex.z > 0) {
					...
					vertex.z = 0;
					auto noise_value = ofNoise(vertex.x * 0.03, vertex.y * 0.03, ofGetFrameNum() * 0.01);
					if (noise_value > 0.35) {

						vertex.z = ofMap(noise_value, 0.35, 1, 0, 300);
					}
				}
			}
			...
			for (int i = index; i < this->face.getNumVertices(); i++) {
				...
				auto color_value = this->face.getVertex(i).z > 0 ? ofMap(this->face.getVertex(i).z, 0, 300, 0, 255) : 0;
				this->face.addColor(ofColor(255, color_value, color_value));
